...
<%		
  ...
  <%if(showpage>1&&showpage<pageallcount){ %>
    <li>
      <a href="/zyc/servlet/ShowArticleImgServlet?currentpage=<%=(showpage-1)%>&id=<%=id %>" aria-label="Previous">
    ...
    if(i==showpage){%>
    <li class="active"><a href="/zyc/servlet/ShowArticleImgServlet?currentpage=<%=showpage%>&id=<%=id %>"><%=i %></a></li>
    <%} 
    else{%>
    <li><a href="/zyc/servlet/ShowArticleImgServlet?currentpage=<%=i%>&id=<%=id %>"><%=i %></a></li>
    <%} %>
    <%} %>
    <%if(showpage<pageallcount){ %>
    <li>
      <a href="/zyc/servlet/ShowArticleImgServlet?currentpage=<%=(showpage+1)%>&id=<%=id %>" aria-label="Next">
        <span aria-hidden="true">下一页</span>
      </a>
    </li>
    <%} %>
	<form class="bs-example bs-example-form" action="/zyc/servlet/ShowArticleImgServlet" method="get" role="form">
