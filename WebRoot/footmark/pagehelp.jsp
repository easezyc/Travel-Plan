...
<%		
  ...
  <%if(showpage>1&&showpage<pageallcount){ %>
    <li>
      <a href="/zyc/servlet/ShowArticleListServlet?currentpage=<%=(showpage-1)%>&search=<%=search %>" aria-label="Previous">
    ...
    if(i==showpage){%>
    <li class="active"><a href="/zyc/servlet/ShowArticleListServlet?currentpage=<%=showpage%>&search=<%=search %>"><%=i %></a></li>
    <%} 
    else{%>
    <li><a href="/zyc/servlet/ShowArticleListServlet?currentpage=<%=i%>&search=<%=search %>"><%=i %></a></li>
    <%} %>
    <%} %>
    <%if(showpage<pageallcount){ %>
    <li>
      <a href="/zyc/servlet/ShowArticleListServlet?currentpage=<%=(showpage+1)%>&search=<%=search %>" aria-label="Next">
        <span aria-hidden="true">下一页</span>
      </a>
    </li>
    <%} %>
	<form class="bs-example bs-example-form" action="/zyc/servlet/ShowArticleListServlet" method="get" role="form">
